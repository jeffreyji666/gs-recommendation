...
import com.ctrip.gs.recommendation.itemcf.precompute.RecommenderJob;
import com.ctrip.gs.recommendation.util.TopItemsQueue;
...
public final class AggregateAndRecommendReducer extends
		...
		if (itemFilePathString != null) {
			...
			for (String line : new FileLineIterable(HadoopUtil.openStream(new Path(itemFilePathString), conf))) {
				try {
					...
					logger.warn("itemsFile line ignored: {}", line);
				}
			}
		}
	}
	...
	private void reduceBooleanData(VarLongWritable userID, Iterable<PrefAndSimilarityColumnWritable> values, Context context)
			throws IOException, InterruptedException {
		/*
	...
	private void reduceNonBooleanData(VarLongWritable userID, Iterable<PrefAndSimilarityColumnWritable> values, Context context)
			throws IOException, InterruptedException {
	 ...
	 */
	private void writeRecommendedItems(VarLongWritable userID, Vector recommendationVector, Context context) throws IOException,
